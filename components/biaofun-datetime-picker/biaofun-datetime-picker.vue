...
<template>
	<view class="datatime">
		<picker mode="multiSelector" v-bind:value="modelVal" :myValue="myValue" :range="range" range-key="text" @change="change" @columnchange="columnchange" :value="value" :disabled="disabled">
			<view v-if="myValue =='' " class="content" :class="{ placeholder: !dateStr }">
				<text>{{ dateStr ? dateStr : placeholder }}</text>
			</view>
			<view v-else class="content" :class="{ placeholder: !dateStr }">
				<text>{{ dateStr ? dateStr : myValue }}</text>
			</view>
		</picker>
	</view>
</template>
	...
	methods: {
		...
		change(event) {
			...
			if(this.fields == 'year') {
				...
				this.$emit('change', utils.formatDate(dtStr).f3);
				return;
			}
				...
				this.setDateStr(dtStr);
				this.$emit('change', utils.formatDate(dtStr).f1);
				return;
			}
